...
class Sentence:
    ...
    def __init__(self, sentence):
        ...
        # dictionary of atomic sentences within sentence, keys are used to link atomic sentences
        self.atomic = {}
        ...
        print("BF Format: ", self.root)
        # final atomic sentence is the root sentence
        self.root = self.__parse(original)
        print("AF Format: ", self.root)
    ...
    def solve(self, model):
        ...
        for key in self.atomic:
            if len(self.atomic[key]) == 2:
                # solve negation
                # get truth value of symbol right of negation
                right = bool_pairs[self.atomic[key][1]]
                # update dictionary with atom key and result of negation
                bool_pairs.update({key: not right})
            elif len(self.atomic[key]) == 3:
                # get truth value left of logic operator
                left = bool_pairs[self.atomic[key][0]]
                # get truth value right of logic operator
                ...
                raise Exception(
                    "Atomic sentence in incorrect format: ", self.atomic[key])
